 ...
 * `value` is the server-known default (provisioned resource id, account
 * email, user-saved default); omitted/blank when the user must fill it.
 */
 ...
 *
 * The optional `role`/`kind`/`timestampMs`/`text`/`fields` block is the
 * programmatic build-completion message the generate route appends when a
 * build finishes (kind 'workflow-done' when every required input is
 * satisfied, 'workflow-done-needs-info' with the outstanding fields when the
 * user still owes values). `timestampMs` is the persisted build-completion
 * time (Date.now() ms). Plain system messages carry `content` only.
 */
export const SystemMessageSchema = BaseMessageSchema.extend({
  ...
  role: z
    .literal('system')
    .optional()
    .describe('Programmatic-origin marker for workflow-done messages'),
  kind: z
    .enum(['workflow-done', 'workflow-done-needs-info'])
    .optional()
    .describe('Build-completion variant'),
  timestampMs: z
    .number()
    .optional()
  ...
  text: z
    .string()
    .optional()
    .describe('Display text of the workflow-done message'),
  fields: z
    .array(SetupFieldDescriptorSchema)
    .optional()
    .describe(
      'Setup form fields (workflow-done-needs-info): the full field list, known values filled, missing required ones blank'
    ),
});
