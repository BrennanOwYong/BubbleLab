...
export const COFFEE_DEFAULT_MODEL = 'openai/gpt-5.2' as const;
...
/**
 * A resource the plan declares must be CREATED for this flow during
 * generation (setup provisioning). The backend creates the resource with the
 * creator's connected credential and writes the created id into the flow's
 * defaultInputs under `inputKey`, so the Setup form is prefilled with a real
 * id. Canonical case: a Google Sheet the flow pipes answers into.
 */
export const SetupResourceSchema = z.object({
  kind: z
    .enum(['google_spreadsheet'])
    .describe('Type of resource to create for this flow'),
  inputKey: z
    .string()
    .min(1)
    .describe(
      'The flow input (payload field) the created resource id fills, e.g. "spreadsheetId"'
    ),
  title: z
    .string()
    .min(1)
    .describe('Human-readable name for the created resource'),
  description: z
    .string()
    .optional()
    .describe('Why this resource exists / what the flow uses it for'),
  sheetTitles: z
    .array(z.string())
    .optional()
    .describe('google_spreadsheet only: initial tab names'),
});

/**
 * A Setup form field descriptor: one flow input the user can see/fill.
 * `header`/`hint` originate from the codegen's @header/@hint JSDoc tags on
 * the payload interface (lifted into the input schema by BubbleParser), with
 * key-humanization/description fallbacks. `value` is the server-known
 * default (provisioned resource id, saved defaultInputs, profile value);
 * omitted/blank when the user must fill it. `fromUserProfile` marks a "for
 * me" field filled from the user's profile (@fromUserProfile tag).
 */
export const SetupFieldDescriptorSchema = z.object({
  key: z.string().describe('Flow input key (payload field name)'),
  header: z.string().describe('Display label for the field'),
  hint: z.string().describe('Help text for the field (may be empty)'),
  value: z
    .string()
    .optional()
    .describe('Known default value; omitted when the field is unfilled'),
  fromUserProfile: z
    .enum(['email', 'telegramChatId'])
    .optional()
    .describe('Profile key this "for me" field is filled from'),
});
...
export const CoffeePlanEventSchema = z.object({
  ...
  setupResources: z
    .array(SetupResourceSchema)
    .optional()
    .describe(
      'Resources that must be created just for this flow during generation (e.g. a Google Sheet the flow writes to), each targeting the flow input its created id fills'
    ),
});
...
// ============================================================================
// Programmatic build-completion messages (workflow-done)
// ============================================================================
// The generate route appends these to metadata.conversationMessages when a
// build finishes. They are discriminated on role+kind (NOT the CoffeeMessage
// `type` field) — the studio's done-message renderer matches this exact
// shape. `timestampMs` is the persisted build-completion time (Date.now()
// ms), durable proof of how long the build took.

export const WorkflowDoneMessageSchema = z.object({
  role: z.literal('system').describe('Programmatic-origin marker'),
  kind: z
    .enum(['workflow-done', 'workflow-done-needs-info'])
    .describe(
      "'workflow-done' when every required input is satisfied; 'workflow-done-needs-info' when the user still owes values"
    ),
  timestampMs: z
    .number()
    .describe('Build-completion time in epoch milliseconds (persisted proof)'),
  text: z.string().describe('Display text of the message'),
  fields: z
    .array(SetupFieldDescriptorSchema)
    .optional()
    .describe(
      'workflow-done-needs-info only: the FULL Setup field list — known values filled, missing required ones blank'
    ),
});

/**
 * One entry of metadata.conversationMessages: a Coffee chat message or a
 * programmatic workflow-done message. The generate route accepts either, so
 * a thread containing done messages round-trips through later rounds.
 */
export const ConversationEntrySchema = z.union([
  CoffeeMessageSchema,
  WorkflowDoneMessageSchema,
]);

/** Narrow a conversation entry to a Coffee chat message (has `type`). */
export function isCoffeeMessage(
  entry: ConversationEntry
): entry is CoffeeMessage {
  return 'type' in entry;
}

/** Narrow a conversation entry to a programmatic workflow-done message. */
export function isWorkflowDoneMessage(
  entry: ConversationEntry
): entry is WorkflowDoneMessage {
  return 'role' in entry && 'kind' in entry;
}
...
export type SetupFieldDescriptor = z.infer<typeof SetupFieldDescriptorSchema>;
...
export type WorkflowDoneMessage = z.infer<typeof WorkflowDoneMessageSchema>;
