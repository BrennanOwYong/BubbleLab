...
export interface OpenApiSecurityScheme {
  ...
  flows?: Record<string, { scopes?: Record<string, string> } | undefined>;
}
...
function inferFromOpenApi(evidence: OpenApiAuthEvidence): AuthInferenceResult {
  const methods: InferredAuthMethod[] = [];
  const uncertainties: AuthInferenceUncertainty[] = [];

  for (const [schemeName, scheme] of Object.entries(evidence.securitySchemes)) {
...
const PROSE_PATTERNS: readonly ProsePattern[] = [
  ...
  },
  {
    ...
    pattern: /\b(log|sign)[- ]?in with your browser\b/i,
  },
